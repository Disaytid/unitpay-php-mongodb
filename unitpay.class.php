...
class UnitPay
{
    ...
    public function getResult()
    {
        ...
        if ($params['sign'] != $this->getMd5Sign($params, UnitpayConfig::SECRET_KEY))
        {
        ...
        if ($method == 'check')
        {
            ...
            $itemsCount = floor($params['sum'] / UnitpayConfig::ITEM_PRICE);

            if ($itemsCount <= 0)
            {
                return $this->getResponseError('Суммы ' . $params['sum'] . ' руб. не достаточно для оплаты товара ' .
                    'стоимостью ' . UnitpayConfig::ITEM_PRICE . ' руб.');
            }
        ...
        if ($method == 'pay')
        {
            ...
            $unitPayModel->confirmPaymentByUnitpayId($params['unitpayId']);
